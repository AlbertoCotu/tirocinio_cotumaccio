...
class Ricetta(models.Model):
	...
	foto = models.ImageField(null=True, blank=True, upload_to="ricette/") #python -m pip install Pillow
...
class MenuSettimana(models.Model):
	...
	creatore = models.ForeignKey(User, on_delete=models.SET_NULL,related_name="creatore", blank=True, null=True) #se user viene cancellato: setto a null
	creazione = models.DateTimeField(default=timezone.now, editable=False)
	approvatore = models.ForeignKey(User, on_delete=models.SET_NULL, limit_choices_to={'is_staff': True}, blank=True, null=True, related_name='approvatore')
